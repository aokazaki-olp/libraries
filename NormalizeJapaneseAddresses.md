...
`ja.json`（ルート）と全市区町村 JSON（約1895ファイル）を1つのオブジェクトに統合。  
キーはファイル名（拡張子なし）、値はそのファイルの内容。

```json
{
  "": { "北海道": ["札幌市中央区", ...], "東京都": ["千代田区", ...], ... },
  "北海道/札幌市中央区": [{ "town": "大通西一丁目", "csv_ranges": {...}, ... }, ...],
  "東京都/渋谷区": [...],
  ...全1895市区町村...
}
```
...
| データ | ファイル数 | 推定サイズ |
|---|---|---|
| `ja.json`（都道府県・市区町村一覧）| 1 | ~370 KB |
| 市区町村別 JSON（レベル3町丁目）| ~1895 | ~33 MB |
| **統合バンドル** | **1** | **~33 MB** |

**格納先の比較:**

| 格納先 | サイズ上限 | Range 読み込み | 評価 |
|---|---|---|---|
| GAS スクリプトファイル内定数 | 50 MB/プロジェクト | 不要 | ✅ 可能だが起動時 JSON.parse が重い |
| Google Drive（単一ファイル）| 制限なし | ❌（全体ロード）| ✅ 実用的。`getBlob().getDataAsString()` で全33MB をメモリに展開 |
| PropertiesService | 9 MB 合計 | 不要 | ❌ 容量不足 |
| CacheService | 100 KB/アイテム | 不要 | ❌ 容量不足 |
