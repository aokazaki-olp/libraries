# 設計レビュー・コードレビュー（第8版 — PR #7 Range サポート 再レビュー）

- **対象**: PR #7 (`claude/add-range-support-LrSSl`) — loadFromSheetAsObjects への Range サポート追加
- **レビュー日**: 2026-02-08
- **レビュー範囲**: loadAsObjects.gs（旧 loadFromSheetAsObjects.gs）, loadAsObjects.test.gs, test-runner.js
...
**修正済み（第4版指摘）**:
- N-4: DELETE リクエストの body を GET/HEAD と同様に警告+無視に変更（RFC 9110 準拠）
...
**修正済み（第4版指摘）**:
- N-3: `extend()` で `config.logger` を渡すように修正（二重ラップ解消）
- N-5: 純粋関数5個を IIFE スコープに移動（毎回再定義を回避）
...
**修正済み（第4版指摘）**:
- N-6: `slackDate`/`slackDateFmt` に `if (v == null) return '';` ガードを追加（null/undefined 対称化）
...
### 2.7 loadAsObjects.gs（309行）— PR #7 で拡張

**品質: A**

旧 `loadFromSheetAsObjects.gs`（217行）を `loadAsObjects.gs`（309行）にリネームし、新たに `loadFromRangeAsObjects` を追加。`loadFromSheetAsObjects` は委譲ラッパーに変更。
...
| 責任範囲の明確化 | 良好 | `resolveRange`（Range 解決）と `resolveSheet`（Sheet 解決）の責任が分離 |
...
GAS の `Range` は `getA1Notation()` を持ち `getSheetId()` を持たない。`Sheet` は逆。否定条件 `getSheetId !== 'function'` は防御的だが、GAS API 上 Sheet は `getA1Notation()` を持たないため本質的には冗長。ただし、将来の GAS API 変更に対する安全マージンとして妥当。

**Range 文字列解決**:
```javascript
const resolveRange = source => {
    if (isRange(source)) return source;
...
`getActiveSpreadsheet()` 依存は `resolveSheet` の文字列モード（シート名指定）と同じ制約。A1 表記（`'A1:D10'`）および `'Sheet2!A1:D10'` 形式をサポート。スタンドアロンスクリプト環境ではアクティブスプレッドシートがないためエラーになるが、これは `resolveSheet` と同一の前提条件。
...
Sheet 全体の Range を取得して `loadFromRangeAsObjects` に委譲。`...args` のパースは `loadFromRangeAsObjects` 内部で行われるため、引数の透過的転送が正しく機能する。パフォーマンスは旧実装と同等（`getValues()` 呼び出し回数は変わらず2回）。

#### 2.7.2 コードレビュー

**良い点**:
- 「切るだけ」の設計原則が引き続き徹底
- 内部関数（`parseSuffix`, `setFlat`, `setNested`）を `toObjects` にリファクタリングし、共通処理を明確化
- `resolveRange` の TypeError がエラーメッセージ付きで投げられ、デバッグ容易性を確保
- `numRows < 1 || numColumns < 1` の早期リターンが正しい
- header-only Range（numRows === 1 → totalDataRows === 0 → rowCount === 0）が正しく空配列を返す

**テストの品質** (`loadAsObjects.test.gs`, 647行):
- MockSheet に Range API メソッド（`getA1Notation`, `getSheet`, `getRow` 等）を追加し、委譲パスのテストを可能に
- MockRange を新設し、任意の開始位置（startRow, startColumn）の Range をシミュレート
- Range オブジェクト、Range 文字列、Sheet 委譲の3パスを網羅
- エラーケース（無効な source 型、無効な Range 文字列）のテストあり
- 約40テストで十分なカバレッジ

---

### 2.8 GoogleSearchConsoleApiClient.gs（83行）
...
**修正済み（第5版指摘）**:
- N-7: `withRetry` に `logger` を追加（リトライログの可視化）
...
#### N-7: GoogleSearchConsoleApiClient.create — withRetry に logger 未指定

**ファイル**: `GoogleSearchConsoleApiClient.gs:73-76`
**重要度**: **L (Low)** — **修正済み**

`HttpCore.withRetry` に `logger` が渡されていなかったため、リトライの各試行ログが出力されていなかった。他の全クライアント（WebhookClient, SlackApiClient, SlackWebhookClient）と同様に `logger` を渡すように修正。

---

### PR #7 新規指摘

#### R-1: test-runner.js がファイルリネームに追従していない

**ファイル**: `test-runner.js:130, 140, 154`
**重要度**: **H (High)** — **修正済み**

`loadFromSheetAsObjects.gs` → `loadAsObjects.gs` のリネーム、および `loadFromSheetAsObjects.test.gs` → `loadAsObjects.test.gs` のリネームに対して、`test-runner.js` が更新されていなかった。

**修正内容** (コミット `0162171`):
- L130: `loadAndRun('loadFromSheetAsObjects.gs')` → `loadAndRun('loadAsObjects.gs')`
- L140: `loadAndRun('loadFromSheetAsObjects.test.gs')` → `loadAndRun('loadAsObjects.test.gs')`
- L154: `{ name: 'loadFromSheetAsObjects', fn: 'runAllLoadFromSheetAsObjectsTests()' }` → `{ name: 'loadAsObjects', fn: 'runAllLoadAsObjectsTests()' }`
...
| loadAsObjects | loadAsObjects.gs | 309 | **A** | PR #7 で Range サポート追加。レイヤー分離は適切。R-1 修正済み |
| GSC Client | GoogleSearchConsoleApiClient.gs | 83 | **A** | responseHandler 統合で簡潔。N-7 修正済み。withGoogleAuth は共通デコレータとして公開（設計意図） |
...
| ID | 重要度 | モジュール | 概要 | ステータス |
|---|---|---|---|---|
| R-1 | High | test-runner.js | ファイルリネームに追従していない（テスト実行不能） | **修正済み** |
...
- **「切るだけ」設計原則**（loadFromRangeAsObjects / loadFromSheetAsObjects）が明確に定義・徹底されている
- GAS V8 ランタイムの制約内で、テスタビリティと拡張性のバランスが取れている

### 9.2 PR #7 レビュー所見

**設計**: Range サポートの追加は、`loadFromRangeAsObjects`（コア）→ `loadFromSheetAsObjects`（ラッパー）のレイヤー分離として適切に実装されている。後方互換性は完全に維持され、既存の利用コードに変更は不要。duck typing による Range 判定、`resolveRange` の文字列サポート、引数の透過的転送も正しい。

**コード品質**: 内部ロジック（`parseSuffix`, `setFlat`, `setNested`）の `toObjects` への抽出は、コードの可読性と保守性を向上させている。テストは約40ケースで十分なカバレッジがあり、MockRange の新設も適切。

**R-1 修正確認済み**: `test-runner.js` のファイル参照3箇所がすべて正しく更新されている（コミット `0162171`）。
...
# 設計レビュー・コードレビュー（第9版 — SlackFilters / SlackResolvers 完全詳細分析）

- **対象**: `SlackFilters.gs`, `SlackResolvers.gs`, `SlackFilters.test.gs`, `SlackResolvers.test.gs`
- **レビュー日**: 2026-02-24
- **レビュー範囲**: SlackFilters・SlackResolvers の設計・コード・コーディング規則適合性・テスト品質
- **変更**: バグ修正 + 規則違反修正 + テスト強化（5ファイル）

---

## 1. SlackFilters.gs — 詳細分析

### 1.1 アーキテクチャ

**品質: A**

IIFE で完全にカプセル化された純粋関数群。LazyTemplate.Filters.Slack 名前空間へのアタッチは、`typeof LazyTemplate === 'undefined'` によるガードで読み込み順序依存を明示する堅牢な設計。

| 区分 | 関数 | 評価 |
|---|---|---|
| Mrkdwn装飾 | bold, italic, strike, code, codeBlock, quote | 全フィルター空文字ガード一貫 ✅ |
| メンション/参照 | mentionUser, mentionChannel, mentionSpecial, link, mail | IDをラップするだけの単責任 ✅ |
| エスケープ | escapeHtml, escapeMrkdwn, escapeJson, escapeBlockKit | 二重エスケープ防止設計が正しい ✅ |
| ユーティリティ | newline, bullet, bulletList, numbered, numberedList, date | processList の再利用が適切 ✅ |

**良い点**:
- 全エクスポート関数が `v => ...` の統一シグネチャで LazyTemplate と完全互換
- `escapeBlockKit = escapeJson(escapeMrkdwn(v))` の合成順序が正しい（`&` の二重エスケープ回避）
- `escapeJson` が制御文字 U+0000〜U+001F を完全カバー（`switch (c)` の `default` で Unicode エスケープ）
- `escapeHtml` の `switch (match)` に `default: return match` があり、CODING_RULES の「switch の default 必須」を遵守

### 1.2 設計上の注意点

**SF-D1 [L/設計]: `link` の JSDoc 分類ミス（SlackFilters.gs:143-148）**
...
`link` 関数の JSDoc コメントが `[メンション]` に分類されているが、`link` は URL リンク（`<https://...>`）であり、メンションではない。セクションコメントの「メンション（参照）系」はやや広義の「参照記法」として許容できるが、JSDoc の `[メンション]` タグは `[リンク]` が正確。

**SF-D2 [L/設計]: `quote` の trailing newline 挙動（SlackFilters.gs:101）**

```javascript
return s.split(/\r?\n/).map(line => `> ${line}`).join('\n');
```

trailing newline（末尾改行）を含む入力は空の引用行 `> ` を生成する。Slack の Mrkdwn レンダリングでは末尾の空引用行が視覚的に余白として表示される場合がある。テストで認識済みだが、必要に応じて `lines.filter(Boolean)` を検討。

**SF-D3 [L/設計]: `date` フィルターと `0` 値（SlackFilters.gs:331-334）**
...
`v = 0` の場合、`toString(0) = '0'` となり `s === ''` 条件を通過して `<!date^0^{date} {time}|0>` を返す。Unix timestamp の `0` は 1970-01-01 00:00:00 UTC を意味するため、これは正しい挙動。テストで明示的に検証済み（`assertEqual('<!date^0^{date} {time}|0>', f(0))`）。

---

## 2. SlackResolvers.gs — 詳細分析

### 2.1 アーキテクチャ

**品質: A-** → 修正後 **A**

IIFE で `create` / `createFromApi` を公開するファクトリパターン。辞書構築とフィルター生成の責任分離が明確で、LazyTemplate との統合も `Object.assign()` によるシンプルな合成で済む。

| 関数 | 責任 | 評価 |
|---|---|---|
| `create(dictionaries)` | 提供済み辞書からフィルター生成 | シンプルで明確 ✅ |
| `createFromApi(slackClient)` | API 全件取得 + 辞書構築 + フィルター生成 | 責任の集約が適切 ✅ |
| `fetchAndBuildUsersMap` | users.list 全ページ取得・辞書構築 | 優先度ロジックが明確 ✅ |
| `fetchAndBuildChannelsMap` | conversations.list 全ページ取得・辞書構築 | シンプルで明確 ✅ |

**優先度ロジックの正しさ**:

`fetchAndBuildUsersMap` は低優先度から順に map へ書き込み、高優先度が上書きする方式を採用。コードの実行順序と「最終的に残る値 = 最優先」の対応が直感に反する可能性はあるが、コメントと JSDoc で明確に説明されており問題なし。

**フェイルセーフ設計**:

未解決の名前が入力値そのまま返る設計は、後続フィルター（`mentionUser` 等）に渡った場合に無効なメンション構文が生成されるトレードオフがある。情報損失を防ぐ設計選択として意図的であり、JSDoc に明記されている。
...
### SR-B1 [H]: GET リクエストで `body` を使用 — pagination パラメータが送信されない **[修正済み]**

**ファイル**: `SlackResolvers.gs:102, 168`（修正前）
...
**根本原因**: `ApiClient.call`（HttpClient.gs:436-448）は GET/HEAD/DELETE リクエストの `body` を無視し、警告ログを出力する。クエリパラメータは `request.query` のみが `buildUrl()` に渡される（HttpClient.gs:426）。

**影響**:
1. `limit: 200` が送信されず、Slack のデフォルト件数（最大200）が使用される（軽微）
2. `cursor`（ページネーション）が送信されない → API は常に1ページ目を返す → **`do...while(cursor)` が無限ループする**

**テスト不備**: テストモックが `options.query` を検証していなかったため、この重大なバグが検出されなかった。テストも合わせて修正（カーソル検証を追加）。
...
同ファイル L180 の `switch (match) {` と不一致。GAS V8 の文法上問題はないが、コードの一貫性を損なう。
...
CODING_RULES §4.2「Null合体演算子 `??` 推奨（`||` と違い `0` や `false` をデフォルト値で上書きしない）」に違反。
...
// 修正後
const usersMap = dictionaries?.users ?? {};      // optional chaining で null 安全性も向上
const channelsMap = dictionaries?.channels ?? {};
return usersMap[s] ?? fallback(s);
return channelsMap[s] ?? fallback(s);
const profile = m.profile ?? {};
...
`dictionaries?.users` の optional chaining は後述 SR-3 の null 対策も兼ねる。

---

### SR-3 [M]: `create(null)` で TypeError **[optional chaining 導入により実質修正済み]**

**ファイル**: `SlackResolvers.gs:43-45`

```javascript
const create = (dictionaries = {}) => {
  const usersMap = dictionaries.users || {};  // ← dictionaries が null → TypeError
```

デフォルト引数 `= {}` は `undefined` のみ適用され、`null` は通過する。`create(null)` 呼び出しで `null.users` の参照により TypeError が発生する。

SR-2 の修正で `dictionaries?.users ?? {}` に変更したことで、`null` を渡しても `undefined ?? {}` → `{}` として安全に処理される。型バリデーション（`TypeError` スロー）を追加する選択肢もあるが、内部的な呼び出しが主であり optional chaining による暗黙的な null 安全性で十分と判断。

---

### ST-1 [L]: テストモックがクエリパラメータを検証していなかった **[修正済み]**

**ファイル**: `SlackResolvers.test.gs`

SR-B1 の根本原因の一つ。`createFromApi` のモックが `options.query?.cursor` を検証せず、単純に呼び出し回数だけをカウントしていた。2回目の呼び出し時に `cursor: 'page2'` が `query` に含まれることを検証するアサーションを追加。

---

## 4. 設計上の観察事項（指摘なし）

### SlackFilters と SlackResolvers の責任分離

`SlackFilters`（装飾: ID → Slack記法）と `SlackResolvers`（解決: 名前 → ID）の責任分離は明確で適切。`Object.assign({}, LazyTemplate.Filters.Slack, resolvers)` による合成パターンにより、利用側でフィルターチェーン `{{{ name | toUserId | mentionUser }}}` が直感的に記述できる。

### `processList` のインライン重複（`numberedList`）

`bulletList` は `processList` ヘルパーを利用しているが、`numberedList` はインデックスが必要なため直接実装している。`processList` にインデックス渡しの拡張は複雑化を招くため、現状の直接実装が適切。

### ページネーションの同期設計

`fetchAndBuildUsersMap` / `fetchAndBuildChannelsMap` の同期ページネーションは、GAS の同期 `UrlFetchApp.fetch` 前提の設計として合理的。`createFromApi` の JSDoc に「大規模ワークスペースではタイムアウトに注意」の記載があり、既知のトレードオフとして明示されている。
...
|---|---|---|---|---|
| SR-B1 | **H (High)** | SlackResolvers.gs:102, 168 | GET リクエストで `body` を使用 → `query` に修正（無限ループ防止） | **修正済み** |
| SF-1 | L (Low) | SlackFilters.gs:21 | アロー関数引数括弧 `(v) =>` → `v =>` | **修正済み** |
| SF-2 | L (Low) | SlackFilters.gs:231 | `switch(c)` → `switch (c)` スペーシング統一 | **修正済み** |
| SR-1 | L (Low) | SlackResolvers.gs:29, 53, 63 | アロー関数引数括弧 `(v) =>` → `v =>` | **修正済み** |
| SR-2 | L (Low) | SlackResolvers.gs:44, 45, 55, 65, 111 | `\|\|` → `??` 演算子統一 | **修正済み** |
| SR-3 | M (Medium) | SlackResolvers.gs:44-45 | `create(null)` の null 安全性（optional chaining 導入で対応） | **修正済み** |
| ST-1 | L (Low) | SlackResolvers.test.gs | モックが `query.cursor` を未検証 → 検証追加 | **修正済み** |
...
|---|---|---|---|---|
| 残存 | 0 | 0 | 0 | **0** |
| 修正済み | 1 (SR-B1) | 1 (SR-3) | 5 (SF-1,2, SR-1,2, ST-1) | 7 |
...
| モジュール | 修正前 | 修正後 | 主な変更 |
|---|---|---|---|
| SlackFilters.gs | A | **A** | アロー関数括弧・switch スペーシング修正（軽微） |
| SlackResolvers.gs | B+ | **A** | 致命的バグ修正（body→query）・null 安全性・規則統一 |
| SlackFilters.test.gs | A | **A** | 変更なし（既存テスト品質が高い） |
| SlackResolvers.test.gs | B | **A-** | ページネーションパラメータの検証追加 |
...
`SlackResolvers.gs` では **SR-B1（High）** として分類した致命的バグを発見・修正した。`users.list` および `conversations.list` への GET リクエストで `body` フィールドにページネーションパラメータを渡していたが、`ApiClient` は GET リクエストの `body` を無視するため、`cursor` が一切送信されず大規模ワークスペースで無限ループが発生する状態であった。`body: params` → `query: params` への変更により修正済み。

加えて、このバグがテストで検出されなかった原因（モックが `query.cursor` を検証していなかった）を修正し、回帰防止のアサーションを追加した。
