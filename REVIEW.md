# 設計レビュー・コードレビュー（第7版 — PR #7 Range サポート レビュー）
...
`loadFromSheetAsObjects.gs` → `loadAsObjects.gs` のリネーム、および `loadFromSheetAsObjects.test.gs` → `loadAsObjects.test.gs` のリネームに対して、`test-runner.js` が更新されていない。`node test-runner.js` を実行すると `ENOENT: no such file or directory` でクラッシュする。

**該当箇所（3箇所）**:

```javascript
// L130: ソースファイル読み込み
loadAndRun('loadFromSheetAsObjects.gs');     // → 'loadAsObjects.gs' に変更必要

// L140: テストファイル読み込み
loadAndRun('loadFromSheetAsObjects.test.gs'); // → 'loadAsObjects.test.gs' に変更必要

// L154: テスト実行
{ name: 'loadFromSheetAsObjects', fn: 'runAllLoadFromSheetAsObjectsTests()' }
// → { name: 'loadAsObjects', fn: 'runAllLoadAsObjectsTests()' } に変更必要
```

**影響**: テストスイート全体が実行不能。CI が存在する場合はパイプライン障害。

---

#### R-2: @version タグが更新されていない

**ファイル**: `loadAsObjects.gs:7`
**重要度**: **L (Low)** — 要修正

ファイルヘッダーの `@version 1.0.0` がコミットメッセージの「v1.1.0」と不一致。Range サポートの追加は機能追加（minor version bump）に相当する。
...
| loadAsObjects | loadAsObjects.gs | 309 | **A-** | PR #7 で Range サポート追加。レイヤー分離は適切。R-1（test-runner 未更新）が要修正 |
...
| ID | 重要度 | モジュール | 概要 | ステータス |
|---|---|---|---|---|
| R-1 | **High** | test-runner.js | ファイルリネームに追従していない（テスト実行不能） | **要修正** |
| R-2 | Low | loadAsObjects.gs | @version タグが 1.0.0 のまま（1.1.0 にすべき） | **要修正** |

### 現在の総計

| 区分 | High | Medium | Low | 合計 |
|---|---|---|---|---|
| PR #7 新規（残存） | 1 (R-1) | 0 | 1 (R-2) | **2** |
| 検討事項 | — | — | — | 1 (N-1) |
| 修正済み | 0 | 0 | 5 (N-3〜N-7) | — |
...
| 優先度 | ID | 対応内容 |
|---|---|---|
| 1 | **R-1** | test-runner.js のファイル参照を `loadAsObjects.gs` / `loadAsObjects.test.gs` / `runAllLoadAsObjectsTests()` に更新 |
| 2 | R-2 | loadAsObjects.gs の `@version` を `1.1.0` に更新 |

R-1 は **マージブロッカー**（テスト実行不能）。R-2 は軽微だが R-1 修正と同時に対応推奨。
...
- **「切るだけ」設計原則**（loadAsObjects）が明確に定義・徹底されている
...
**要修正**: `test-runner.js` がファイルリネームに追従しておらず（R-1, High）、テストスイート全体が実行不能。これは **マージブロッカー** であり、早急な修正が必要。`@version` タグの不一致（R-2, Low）も合わせて修正推奨。
